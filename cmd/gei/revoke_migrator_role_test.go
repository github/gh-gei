...
// mockMigratorRoleRevoker implements migratorRoleRevoker for testing.
type mockMigratorRoleRevoker struct {
	orgID        string
	orgIDErr     error
	revokeResult bool
	revokeErr    error
...
func (m *mockMigratorRoleRevoker) GetOrganizationId(_ context.Context, org string) (string, error) {
	m.gotOrg = org
	return m.orgID, m.orgIDErr
}

func (m *mockMigratorRoleRevoker) RevokeMigratorRole(_ context.Context, orgID, actor, actorType string) (bool, error) {
	m.gotOrgID = orgID
	m.gotActor = actor
	m.gotType = actorType
...
func TestRevokeMigratorRole(t *testing.T) {
	tests := []struct {
		name       string
		args       []string
		mock       *mockMigratorRoleRevoker
		wantErr    string
		wantOutput []string
		assertArgs func(t *testing.T, m *mockMigratorRoleRevoker)
	}{
		{
			name: "revoke succeeds",
			args: []string{"--github-org", "my-org", "--actor", "monalisa", "--actor-type", "USER"},
			mock: &mockMigratorRoleRevoker{orgID: "ORG_ID_123", revokeResult: true},
			wantOutput: []string{
				"Revoking migrator role ...",
				`Migrator role successfully revoked for the USER "monalisa"`,
			},
			assertArgs: func(t *testing.T, m *mockMigratorRoleRevoker) {
		...
		{
			name: "revoke succeeds with lowercase actor type",
			args: []string{"--github-org", "my-org", "--actor", "my-team", "--actor-type", "team"},
			mock: &mockMigratorRoleRevoker{orgID: "ORG_ID_123", revokeResult: true},
			wantOutput: []string{
				`Migrator role successfully revoked for the TEAM "my-team"`,
			},
		},
		{
			name: "revoke fails returns false",
			args: []string{"--github-org", "my-org", "--actor", "monalisa", "--actor-type", "USER"},
			mock: &mockMigratorRoleRevoker{orgID: "ORG_ID_123", revokeResult: false},
			wantOutput: []string{
				`Migrator role couldn't be revoked for the USER "monalisa"`,
			},
		},
		{
			name:    "GetOrganizationId error propagates",
			args:    []string{"--github-org", "my-org", "--actor", "monalisa", "--actor-type", "USER"},
			mock:    &mockMigratorRoleRevoker{orgIDErr: fmt.Errorf("org not found")},
		...
		{
			name:    "RevokeMigratorRole error propagates",
			args:    []string{"--github-org", "my-org", "--actor", "monalisa", "--actor-type", "USER"},
			mock:    &mockMigratorRoleRevoker{orgID: "ORG_ID_123", revokeErr: fmt.Errorf("permission denied")},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			cmd := newRevokeMigratorRoleCmd(tc.mock, log)
			cmd.SetOut(&buf)
			cmd.SetErr(&buf)
			cmd.SetArgs(tc.args)
