...
// Requires PROXIMA_PAT and GHEC_PAT secrets to be set. The source repo is treated as read-only
// (tenant-managed); only the target org is reset each run.
...
public sealed class ProximaToGithub : IDisposable
{
    ...
    public ProximaToGithub(ITestOutputHelper output)
    {
        ...
        TestHelper.AssertCredentialsPresent(
            ("PROXIMA_PAT", "GitHub Enterprise Cloud with data residency (source) personal access token"),
            ("GHEC_PAT", "GitHub Enterprise Cloud (target) personal access token"));

        var logger = new OctoLogger(_ => { }, x => _output.WriteLine(x), _ => { }, _ => { });

        var sourceGithubToken = Environment.GetEnvironmentVariable("PROXIMA_PAT");
        ...
        var retryPolicy = new RetryPolicy(logger, "Proxima (PROXIMA_PAT)");
        var environmentVariableProvider = new EnvironmentVariableProvider(logger);

        _sourceGithubHttpClient = new HttpClient();
        _sourceGithubClient = new GithubClient(logger, _sourceGithubHttpClient, new VersionChecker(_versionClient, logger), new RetryPolicy(logger, "Proxima (PROXIMA_PAT)"), new DateTimeProvider(), sourceGithubToken);
        _archiveUploader = new ArchiveUploader(_targetGithubClient, UPLOADS_URL, logger, retryPolicy, environmentVariableProvider);
        _sourceGithubApi = new GithubApi(_sourceGithubClient, PROXIMA_API_URL, new RetryPolicy(logger, "Proxima (PROXIMA_PAT)"), _archiveUploader);
