...
public sealed class BbsToGithub : IDisposable
{
    ...
    public BbsToGithub(ITestOutputHelper output)
    {
        ...
        File.WriteAllText(SSH_KEY_FILE, sshKey);
    ...
    {
        var bbsSourceProject = $"e2e-testing-{TestHelper.GetOsName()}";
        ...
        await _targetHelper.RunBbsCliMigration(
            $"generate-script --github-target-org {githubTargetOrg} --bbs-server-url {BBS_URL} --ssh-user octoshift --ssh-private-key {SSH_KEY_FILE}", _tokens);
