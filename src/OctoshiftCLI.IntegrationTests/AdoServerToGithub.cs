...
namespace OctoshiftCLI.IntegrationTests
{
    ...
    public class AdoServerToGithub : AdoToGithub
    {
        ...
        public async Task Basic()
        {
            ...
            await Helper.AssertAutolinkConfigured(githubOrg, $"{teamProject1}-{teamProject1}", $"{ADO_SERVER_URL}/{adoOrg}/{teamProject1}/_workitems/edit/<num>/");
            await Helper.AssertAutolinkConfigured(githubOrg, $"{teamProject2}-{teamProject2}", $"{ADO_SERVER_URL}/{adoOrg}/{teamProject2}/_workitems/edit/<num>/");
            await Helper.AssertAdoRepoDisabled(adoOrg, teamProject1, adoRepo1, ADO_SERVER_URL);
            await Helper.AssertAdoRepoDisabled(adoOrg, teamProject2, adoRepo2, ADO_SERVER_URL);
            await Helper.AssertAdoRepoLocked(adoOrg, teamProject1, adoRepo1, ADO_SERVER_URL);
            await Helper.AssertAdoRepoLocked(adoOrg, teamProject2, adoRepo2, ADO_SERVER_URL);
            ...
            await Helper.AssertServiceConnectionWasShared(adoOrg, teamProject1, ADO_SERVER_URL);
            await Helper.AssertServiceConnectionWasShared(adoOrg, teamProject2, ADO_SERVER_URL);
            await Helper.AssertPipelineRewired(adoOrg, teamProject1, pipeline1, githubOrg, $"{teamProject1}-{teamProject1}", ADO_SERVER_URL);
            await Helper.AssertPipelineRewired(adoOrg, teamProject2, pipeline2, githubOrg, $"{teamProject2}-{teamProject2}", ADO_SERVER_URL);
