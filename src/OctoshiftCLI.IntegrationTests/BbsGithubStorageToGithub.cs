...
public sealed class BbsGithubStorageToGithub : IDisposable
{
    ...
    public BbsGithubStorageToGithub(ITestOutputHelper output)
    {
    ...
    [Fact]
    public async Task MigrateRepo_BbsLinux_GithubStorage()
    {
        ...
        // Use GitHub Storage
        var githubStorageOptions = " --use-github-storage";

        await _targetHelper.RunBbsCliMigration(
            $"migrate-repo --github-org {githubTargetOrg} --bbs-server-url {bbsServer} --bbs-project {bbsProjectKey} --bbs-repo {repo1} --github-repo {targetRepo1}{archiveDownloadOptions}{githubStorageOptions}", _tokens);
