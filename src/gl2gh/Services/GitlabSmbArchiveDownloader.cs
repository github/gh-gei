...
public sealed class GitlabSmbArchiveDownloader : IGitlabArchiveDownloader
{
    ...
    public GitlabSmbArchiveDownloader(OctoLogger log, FileSystemProvider fileSystemProvider, string host, string smbUser, string smbPassword, string domainName = null)
        : this(log, fileSystemProvider, new SMB2Client(), host, smbUser, smbPassword, domainName)
    {
    }

    internal GitlabSmbArchiveDownloader(
        OctoLogger log,
    ...
    public string GitlabSharedHomeDirectory { get; init; } = GitlabSettings.DEFAULT_BBS_SHARED_HOME_DIRECTORY_WINDOWS;

    private string GetSourceExportArchiveAbsolutePath(long exportJobId) =>
        IGitlabArchiveDownloader.GetSourceExportArchiveAbsolutePath(GitlabSharedHomeDirectory ?? GitlabSettings.DEFAULT_BBS_SHARED_HOME_DIRECTORY_WINDOWS, exportJobId).ToWindowsPath();

    public async Task<string> Download(long exportJobId, string targetDirectory = IGitlabArchiveDownloader.DEFAULT_TARGET_DIRECTORY)
    {
        ...
        var targetExportArchiveFullPath =
            Path.Join(targetDirectory ?? IGitlabArchiveDownloader.DEFAULT_TARGET_DIRECTORY, IGitlabArchiveDownloader.GetExportArchiveFileName(exportJobId)).ToUnixPath();
    ...
    private object CreateFileHandle(ISMBFileStore fileStore, string sharedFilePath)
    {
        ...
        return IsSuccessStatus(status)
            ...
            : throw new OctoshiftCliException(
                ...
                (IsObjectPathNotFoundStatus(status) && GitlabSharedHomeDirectory is GitlabSettings.DEFAULT_BBS_SHARED_HOME_DIRECTORY_WINDOWS
