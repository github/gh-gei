...
public sealed class GitlabSshArchiveDownloader : IGitlabArchiveDownloader, IDisposable
{
    ...
    public GitlabSshArchiveDownloader(OctoLogger log, FileSystemProvider fileSystemProvider, string host, string sshUser, string privateKeyFileFullPath, int sshPort = 22)
    {
        _log = log;
    ...
    internal GitlabSshArchiveDownloader(OctoLogger log, FileSystemProvider fileSystemProvider, ISftpClient sftpClient)
    {
        _log = log;
        _fileSystemProvider = fileSystemProvider;
    ...
    private string GetSourceExportArchiveAbsolutePath(long exportJobId) =>
        IGitlabArchiveDownloader.GetSourceExportArchiveAbsolutePath(GitlabSharedHomeDirectory ?? GitlabSettings.DEFAULT_BBS_SHARED_HOME_DIRECTORY_LINUX, exportJobId).ToUnixPath();
    ...
    public async Task<string> Download(long exportJobId, string targetDirectory = IGitlabArchiveDownloader.DEFAULT_TARGET_DIRECTORY)
    {
        ...
        if (!_sftpClient.Exists(sourceExportArchiveFullPath))
        {
            throw new OctoshiftCliException(
                ...
                (GitlabSharedHomeDirectory is GitlabSettings.DEFAULT_BBS_SHARED_HOME_DIRECTORY_LINUX
