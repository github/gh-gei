...
namespace OctoshiftCLI.Tests.GitlabToGithub.Commands.MigrateRepo
{
    public class MigrateRepoCommandHandlerTests
    {
        ...
        private readonly Mock<IGitlabArchiveDownloader> _mockGitlabArchiveDownloader = new();
        ...
        public MigrateRepoCommandHandlerTests()
        {
            ...
            _handler = new MigrateRepoCommandHandler(
                ...
                _mockGitlabArchiveDownloader.Object,
        ...
        {
            // Arrange
            _mockGitlabApi.Setup(x => x.StartExport(BBS_PROJECT, BBS_REPO)).ReturnsAsync(BBS_EXPORT_ID);
            _mockGitlabApi.Setup(x => x.GetExport(BBS_EXPORT_ID)).ReturnsAsync(("COMPLETED", "The export is complete", 100));
            ...
            // Assert
            _mockGitlabApi.Verify(m => m.StartExport(
                BBS_PROJECT,
                BBS_REPO
            ));
        ...
        public async Task Happy_Path_Generate_And_Download()
        {
            ...
            // Assert
            _mockGitlabArchiveDownloader.Verify(m => m.Download(BBS_EXPORT_ID, It.IsAny<string>()));
            _mockGithubApi.Verify(m => m.DoesRepoExist(It.IsAny<string>(), It.IsAny<string>()), Times.Never);
        }
        ...
        public async Task Happy_Path_Ingest_Only()
        {
            ...
            _mockGithubApi.Setup(x => x.CreateGitlabMigrationSource(GITHUB_ORG_ID).Result).Returns(MIGRATION_SOURCE_ID);
        ...
        public async Task Happy_Path_Generate_Archive_Ssh_Download_Azure_Upload_And_Ingest()
        {
            ...
            _mockGitlabApi.Setup(x => x.StartExport(BBS_PROJECT, BBS_REPO)).ReturnsAsync(BBS_EXPORT_ID);
            _mockGitlabApi.Setup(x => x.GetExport(BBS_EXPORT_ID)).ReturnsAsync(("COMPLETED", "The export is complete", 100));
            _mockGitlabArchiveDownloader.Setup(x => x.Download(BBS_EXPORT_ID, It.IsAny<string>())).ReturnsAsync(ARCHIVE_PATH);
        ...
        [Fact]
        public async Task Happy_Path_Full_Flow_Running_On_Gitlab_Server()
        {
            // Arrange
            ...
            var args = new MigrateRepoCommandArgs
            {
                ...
                GithubRepo = GITHUB_REPO,
                GithubPat = GITHUB_PAT,
                GitlabSharedHome = bbsSharedHome,
                QueueOnly = true,
            };
        ...
        [Fact]
        public async Task Happy_Path_Full_Flow_Gitlab_Credentials_Via_Environment()
        {
            // Arrange
        ...
        public async Task Dont_Generate_Archive_If_Target_Repo_Exists()
        {
            ...
            // Assert
            _mockGitlabApi.Verify(x => x.StartExport(It.IsAny<string>(), It.IsAny<string>()), Times.Never);
        }
        ...
        public async Task Uses_GitHub_Pat_When_Provided_As_Option()
        {
            ...
            _mockGithubApi.Setup(x => x.CreateGitlabMigrationSource(GITHUB_ORG_ID).Result).Returns(MIGRATION_SOURCE_ID);
            _mockGithubApi
                .Setup(x => x.StartGitlabMigration(MIGRATION_SOURCE_ID, UNUSED_REPO_URL, GITHUB_ORG_ID, GITHUB_REPO, GITHUB_PAT, ARCHIVE_URL, null).Result)
        ...
        public async Task Throws_Decorated_Error_When_Create_Migration_Source_Fails_With_Permissions_Error()
        {
            ...
            _mockGithubApi.Setup(x => x.GetOrganizationId(GITHUB_ORG).Result).Returns(GITHUB_ORG_ID);
            _mockGithubApi
                .Setup(x => x.CreateGitlabMigrationSource(GITHUB_ORG_ID).Result)
        ...
        public async Task Throws_An_Error_If_Export_Fails()
        {
            // Arrange
            _mockGitlabApi.Setup(x => x.StartExport(BBS_PROJECT, BBS_REPO)).ReturnsAsync(BBS_EXPORT_ID);
            _mockGitlabApi.Setup(x => x.GetExport(BBS_EXPORT_ID)).ReturnsAsync(("FAILED", "The export failed", 0));
        ...
        [Fact]
        public async Task Invoke_With_Gitlab_Server_Url_Throws_When_Smb_User_Is_Provided_And_Smb_Password_Is_Not_Provided()
        {
            // Act, Assert
            var args = new MigrateRepoCommandArgs
            {
        ...
        [Fact]
        public async Task Invoke_With_Gitlab_Server_Url_Should_Not_Throw_When_Smb_User_Is_Provided_And_Smb_Password_Is_Provided_Via_Environment_Variable()
        {
            // Arrange
            _mockGitlabApi.Setup(x => x.StartExport(BBS_PROJECT, BBS_REPO)).ReturnsAsync(BBS_EXPORT_ID);
            _mockGitlabApi.Setup(x => x.GetExport(BBS_EXPORT_ID)).ReturnsAsync(("COMPLETED", "The export is complete", 100));
        ...
        [Fact]
        public async Task Errors_If_GitlabServer_Url_Provided_But_No_Gitlab_Password()
        {
            // Act
            var args = new MigrateRepoCommandArgs
            {
        ...
        [Fact]
        public async Task It_Should_Not_Validate_Gitlab_Username_And_Password_When_Kerberos_Is_Set()
        {
            // Arrange
            _mockGitlabApi.Setup(x => x.GetExport(It.IsAny<long>())).ReturnsAsync(("COMPLETED", "The export is complete", 100));
        ...
        [Fact]
        public async Task It_Throws_When_Gitlab_Shared_Home_Does_Not_Exist_When_Running_On_Bitbucket_Instance()
        {
            const string nonExistentGitlabSharedHome = "/nonexistent/shared/home";
            _mockGitlabApi.Setup(x => x.StartExport(BBS_PROJECT, BBS_REPO)).ReturnsAsync(BBS_EXPORT_ID);
            _mockGitlabApi.Setup(x => x.GetExport(BBS_EXPORT_ID)).ReturnsAsync(("COMPLETED", "The export is complete", 100));
            _mockFileSystemProvider.Setup(m => m.DirectoryExists(nonExistentGitlabSharedHome)).Returns(false);
            ...
            await _handler.Invoking(async x => await x.Handle(new MigrateRepoCommandArgs
            {
                ...
                GitlabProject = BBS_PROJECT,
                GitlabRepo = BBS_REPO,
                GitlabSharedHome = nonExistentGitlabSharedHome
            }))
                .Should()
                .ThrowAsync<OctoshiftCliException>()
                .WithMessage($"*--bbs-shared-home*{nonExistentGitlabSharedHome}*");
        }

        [Fact]
        public async Task It_Does_Not_Validate_Gitlab_Shared_Home_When_Using_Ssh()
        {
            const string nonExistentGitlabSharedHome = "/nonexistent/shared/home";
            _mockGitlabApi.Setup(x => x.StartExport(BBS_PROJECT, BBS_REPO)).ReturnsAsync(BBS_EXPORT_ID);
            _mockGitlabApi.Setup(x => x.GetExport(BBS_EXPORT_ID)).ReturnsAsync(("COMPLETED", "The export is complete", 100));
        ...
        [Fact]
        public async Task It_Does_Not_Validate_Gitlab_Shared_Home_When_Using_Smb()
        {
            const string nonExistentGitlabSharedHome = "/nonexistent/shared/home";
            _mockGitlabApi.Setup(x => x.StartExport(BBS_PROJECT, BBS_REPO)).ReturnsAsync(BBS_EXPORT_ID);
            _mockGitlabApi.Setup(x => x.GetExport(BBS_EXPORT_ID)).ReturnsAsync(("COMPLETED", "The export is complete", 100));
