...
public class LfsShaMapperTests
{
    ...
    public LfsShaMapperTests() =>
        ...
        {
            WriteToFile = (_, contents) => Task.CompletedTask,
            ReadFile = _ => "Bunch of text with oldsha all over the place. oldsha here, oldsha there, oldsha everywhere.",
            ReadMappingFile = _ => new List<string> { "oldsha,newsha" }
        };
    ...
    public async Task Does_This_Work()
    {
        ...
        _archiveHandler.Setup(m => m.Unpack(It.IsAny<byte[]>()))
            .Returns(new[] { "archiveExtracted/pull_requests_1.json", "archiveExtracted/pull_requests_2.json" });
