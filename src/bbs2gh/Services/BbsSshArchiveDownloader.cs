...
#pragma warning restore CA2000
    ...
    public async Task<string> Download(long exportJobId, string targetDirectory = IBbsArchiveDownloader.DEFAULT_TARGET_DIRECTORY)
    {
        ...
        var sourceExportArchiveFullPath = Path.Join(BbsSharedHomeDirectory, IBbsArchiveDownloader.GetSourceExportArchiveRelativePath(exportJobId)).Replace('\\', '/');
        var targetExportArchiveFullPath = Path.Join(targetDirectory, IBbsArchiveDownloader.GetExportArchiveFileName(exportJobId)).Replace('\\', '/');
