...
namespace OctoshiftCLI.AdoToGithub
{
    public class ReposCsvGeneratorService
    {
        ...
        public virtual async Task<string> Generate(string adoPat, bool minimal = false)
        {
            ...
            result.Append("org,teamproject,repo,url,last-push-date,compressed-repo-size-in-bytes");
            result.AppendLine(!minimal ? ",most-active-contributor,pipeline-count,pr-count,commits-past-year" : null);
            ...
            foreach (var org in await inspector.GetOrgs())
            {
                foreach (var teamProject in await inspector.GetTeamProjects(org))
                {
                    foreach (var repo in await inspector.GetRepos(org, teamProject))
                    {
                        ...
                        var pipelineCount = !minimal ? await inspector.GetPipelineCount(org, teamProject, repo.Name) : 0;
                        ...
                        result.Append($"\"{org}\",\"{teamProject}\",\"{repo.Name}\",\"{url}\",\"{lastPushDate:dd-MMM-yyyy hh:mm tt}\",\"{repo.Size:N0}\"");
                        result.AppendLine(!minimal ? $",\"{mostActiveContributor}\",{pipelineCount},{prCount},{commitsPastYear}" : null);
                    }
                }
            }
