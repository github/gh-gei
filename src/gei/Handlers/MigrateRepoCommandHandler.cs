...
public class MigrateRepoCommandHandler : ICommandHandler<MigrateRepoCommandArgs>
{
    ...
    {
        var gitSourceMigrationApiId = await _sourceGithubApi.StartGitArchiveGeneration(githubSourceOrg, sourceRepo);
        _log.LogInformation($"Archive generation of git data started with id: {gitSourceMigrationApiId}");
        ...
        var gitArchiveFileName = $"{timeNow}-{gitSourceMigrationApiId}-{GIT_ARCHIVE_FILE_NAME}";
        var metadataArchiveFileName = $"{timeNow}-{metadataArchiveId}-{METADATA_ARCHIVE_FILE_NAME}";

        var gitArchiveUrl = await WaitForArchiveGeneration(_sourceGithubApi, githubSourceOrg, gitSourceMigrationApiId);
    ...
    private async Task<string> WaitForArchiveGeneration(GithubApi githubApi, string githubSourceOrg, int sourceMigrationApiId)
    {
        var timeout = DateTime.Now.AddHours(ARCHIVE_GENERATION_TIMEOUT_IN_HOURS);
        while (DateTime.Now < timeout)
        {
            var archiveStatus = await githubApi.GetArchiveMigrationStatus(githubSourceOrg, sourceMigrationApiId);
            _log.LogInformation($"Waiting for archive with Migration ID {sourceMigrationApiId} generation to finish. Current status: {archiveStatus}");
            if (archiveStatus == ArchiveMigrationStatus.Exported)
            {
                return await githubApi.GetArchiveMigrationUrl(githubSourceOrg, sourceMigrationApiId);
            }
            if (archiveStatus == ArchiveMigrationStatus.Failed)
            {
                throw new OctoshiftCliException($"Archive generation failed for Source Migration API ID: {sourceMigrationApiId}");
            }
